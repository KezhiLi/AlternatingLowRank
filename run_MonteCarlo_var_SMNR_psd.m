...
store_rank_x           = zeros(M_runs,length(SMNR_list));
store_rank_x_ls        = zeros(M_runs,length(SMNR_list));
store_rank_x_ls_struct = zeros(M_runs,length(SMNR_list));

store_minsvalue_x           = zeros(M_runs,length(SMNR_list));
store_minsvalue_x_ls        = zeros(M_runs,length(SMNR_list));
store_minsvalue_x_ls_struct = zeros(M_runs,length(SMNR_list));

store_bias_x_ls        = zeros(M_runs,length(SMNR_list));
store_bias_x_ls_struct = zeros(M_runs,length(SMNR_list));
...
for j = 1:M_runs
    ...
    tol_struct = 1e-13; %tolerance for structured matrix
...
%% Save data
save MC_results_SMNR_001_psd   N P K x_struct rho M_runs SMNR_list SMNR_emp   SRER_ls SRER_ls_struct SRER_crlb SRER_crlb_struct  sq_x sq_n  sq_e_ls sq_e_ls_struct sq_e_crlb sq_e_crlb_struct
